...
input[type=checkbox]#cb-all-donors,input[type=checkbox]#cb-all-requests,input[type=checkbox]#cb-all-reports,input[type=checkbox]#cb-all-calls,input[type=checkbox]#cb-all-secretreqs{width:15px;height:15px;cursor:pointer;accent-color:var(--red);}
      ...
      <div class="tbox">
        <div class="tbar">
          ...
          <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;">
            ...
            <?php if($is_super):?>
            <button onclick="clearSecretReqs('all')" style="padding:5px 14px;background:rgba(239,68,68,.12);color:var(--red);border:1px solid rgba(239,68,68,.25);border-radius:8px;font-size:.78em;font-weight:700;cursor:pointer;">🗑 সব মুছুন</button>
            <button onclick="clearSecretReqs('processed')" style="padding:5px 14px;background:rgba(107,114,128,.1);color:var(--muted);border:1px solid var(--bdr);border-radius:8px;font-size:.78em;cursor:pointer;">🗑 Processed মুছুন</button>
            <?php endif;?>
          </div>
        </div>
        <div class="bulk-bar" id="bulk-bar-secretreqs" <?php if($is_moderator) echo 'style="display:none!important;"'; ?>>
          <span>✅ <strong id="secretreqs-sel-count">0</strong> টি selected</span>
          <button class="bulk-del-btn" onclick="bulkDelete('secretreqs')">🗑 Delete Selected</button>
          <button class="bulk-cancel-btn" onclick="clearSelection('secretreqs')">✕ Cancel</button>
        </div>
        <div class="ow"><table>
          <thead><tr>
            <th class="cb-th"><input type="checkbox" id="cb-all-secretreqs" onchange="toggleAll('secretreqs',this.checked)"></th>
            <th>#</th><th>Donor Name</th><th>Phone</th><th>Ref Code</th><th>Device ID</th><th>IP</th><th>Status</th><th>Requested</th><th>Actions</th></tr></thead>
          <tbody id="scReqTable"><tr><td colspan="10" class="empty">⏳ Load করুন...</td></tr></tbody>
        </table></div>
      </div>
    </div>
...
function loadSecretReqs(){
    ...
    tb.innerHTML='<tr><td colspan="10" class="empty">⏳ লোড হচ্ছে...</td></tr>';
    const fd=new FormData(); fd.append('act','get_secret_requests'); fd.append('csrf',CSRF);
    fetch(window.location.href,{method:'POST',body:fd,headers:{'X-Requested-With':'XMLHttpRequest'}})
    .then(r=>r.json()).then(d=>{
        if(!d.ok||!d.rows.length){ tb.innerHTML='<tr><td colspan="10" class="empty">কোনো request নেই</td></tr>'; return; }
        const statusColor={'pending':'#f59e0b','approved':'#10b981','denied':'#ef4444','ref_expired':'#6b7280'};
        tb.innerHTML=d.rows.map((r,i)=>`
            <tr id="scr_${r.id}" class="secretreqs-row">
              <td class="cb-td"><input type="checkbox" class="row-cb secretreqs-cb" value="${r.id}" onchange="onRowCbChange('secretreqs')"></td>
              <td style="color:var(--muted);">${i+1}</td>
              ...
              <td>
                ...
                ${IS_SUPER?`<button class="btn bd" onclick="act('del_secretreq',${r.id},this,'scr_${r.id}')" title="Delete" style="padding:3px 7px;font-size:.72em;">🗑</button>`:''}
                </div>
              </td>
            </tr>`).join('');
    }).catch(()=>{ tb.innerHTML='<tr><td colspan="10" class="empty">❌ Load করতে সমস্যা হয়েছে।</td></tr>'; });
}


function clearSecretReqs(filter){
    var lbl=filter==='processed'?'Processed (approved/denied/expired) requests':'সব requests';
    if(!confirm('🗑 '+lbl+' মুছে ফেলবেন?')) return;
    var fd=new FormData(); fd.append('act','clear_secret_reqs'); fd.append('filter',filter); fd.append('csrf',CSRF);
    fetch(window.location.href,{method:'POST',body:fd,headers:{'X-Requested-With':'XMLHttpRequest'}})
